...
export default function Testimonials() {
    ...
    return (
        <>
            ...
            <section style={{ padding: '120px 40px', background: '#0a0a0c', fontFamily: "'Poppins', sans-serif", position: 'relative', overflow: 'hidden' }}>

                {/* Decorative */}
                <div style={{ position: 'absolute', top: '-80px', left: '-80px', width: '400px', height: '400px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(151,71,255,0.07) 0%, transparent 70%)', filter: 'blur(60px)', pointerEvents: 'none' }} />
                ...
                <div style={{ maxWidth: '1280px', margin: '0 auto', position: 'relative' }}>
                    ...
                    <div style={{ marginBottom: '60px' }}>
                        ...
                        <h2 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: '800', color: '#ffffff', margin: '0 0 16px 0', letterSpacing: '-0.025em', lineHeight: 1.15 }}>
                            Success Stories<br />
                            <span style={{ background: 'linear-gradient(95deg, #9747FF, #c084fc)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>From the Field</span>
                        </h2>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '24px', alignItems: 'start' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {reviews.map((r, i) => (
                                <button key={i}
                                    ...
                                    <div>
                                        <div style={{ fontSize: '13px', fontWeight: '700', color: selected === i ? '#ffffff' : 'rgba(255,255,255,0.65)', lineHeight: 1.3 }}>{r.title}</div>
                                        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginTop: '2px' }}>{r.industry}</div>
                                    </div>
                                </button>
                            ))}
                        </div>
                        ...
                        <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '24px', padding: '40px', display: 'flex', flexDirection: 'column', gap: '24px' }}>

                            {/* Quote mark */}
                            <div style={{ fontSize: '64px', lineHeight: 1, color: active.color, opacity: 0.3, fontFamily: 'Georgia, serif', marginBottom: '-16px' }}>"</div>

                            {/* Quote */}
                            <p style={{ fontSize: '16px', fontStyle: 'italic', color: 'rgba(255,255,255,0.75)', lineHeight: 1.8, margin: 0 }}>
                                {active.quote}
                            </p>
                            ...
                            {/* Bottom row: attribution + stat */}
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap' }}>
                                <div>
                                    <div style={{ fontSize: '14px', fontWeight: '700', color: '#ffffff' }}>— Verified Partner Executive</div>
                                    <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', marginTop: '3px' }}>{active.service}</div>
                                </div>
                                <div style={{ background: `${active.color}18`, border: `1px solid ${active.color}33`, borderRadius: '14px', padding: '14px 22px', textAlign: 'center' }}>
                                    ...
                                    <div style={{ fontSize: '10px', fontWeight: '700', color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '0.12em', marginTop: '4px' }}>{active.metricLabel}</div>
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', paddingTop: '8px' }}>
                                {reviews.map((r, i) => (
                                    <button key={i} className="tes-nav-dot"
                                        onClick={() => setSelected(i)}
                                        style={{ background: selected === i ? r.color : 'rgba(255,255,255,0.15)' }} />
                                ))}
                            </div>
                        </div>
                    </div>
